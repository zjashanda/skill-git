---
name: skill-git
description: Publish local Codex skills to GitHub with one token and one SSH transport, using the skill name as the repository name. Use when Codex needs to create or verify a GitHub repo for a skill, export a local skill into a same-name repository, generate a GitHub README from SKILL.md, commit and push updates, or explain the standard skill publishing workflow.
---
